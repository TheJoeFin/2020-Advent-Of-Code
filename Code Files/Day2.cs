...
namespace _2020_Advent_Of_Code
{
    public class Day2
    {
        public static void Run()
        {
            ...
            List<string> correctPasswords = new List<string>();
            List<string> incorrectPasswords = new List<string>();

            foreach (string pword in passwordInputs)
            {
                string[] segmentedPWord = pword.Split();

                string[] rangeAsStrings = segmentedPWord[0].Split('-');

                char requiredChar = segmentedPWord[1].Replace(":", "").FirstOrDefault();

                string passwordToCheck = segmentedPWord[2];
                ...
                if (requiredChar == passwordToCheck[positionOne - 1])
                    posOneCheck = true;

                if (requiredChar == passwordToCheck[positionTwo - 1])
                    posTwoCheck = true;

                if (posOneCheck == true && posTwoCheck == true)
                    incorrectPasswords.Add(pword);
                else if (posOneCheck == true && posTwoCheck == false)
                {
                    numberOfCorrectPasswords++;
                    correctPasswords.Add(pword);
                }
                else if (posOneCheck == false && posTwoCheck == true)
                {
                    numberOfCorrectPasswords++;
                    correctPasswords.Add(pword);
                }
            ...
            Console.WriteLine($"Number of correct passwords: {numberOfCorrectPasswords}");

            foreach (string correctPWORD in correctPasswords.Take(5))
            {
                Console.WriteLine(correctPWORD);
            }

            Console.WriteLine("\n\nIncorrect Examples:");
            foreach (string incorrectPWORD in incorrectPasswords.Take(5))
            {
                Console.WriteLine(incorrectPWORD);
            }

        }

        private static List<string> passwordInputs = new List<string>{
        "1-2 x: xpxc",
    "1-5 b: bwlbbbbcq",
    "3-5 v: qvjjdhvl",
