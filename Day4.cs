...
namespace _2020_Advent_Of_Code
{
    ...
    public class Day4
    {
        public static void Run()
        {
            ...
            List<Passport> GoodPassports = new List<Passport>();
            List<Passport> BadPassports = new List<Passport>();

            Passport newPassport = new Passport();

            foreach (string inputLine in splitInput)
            {
                string displayLine = inputLine.Trim();

                string[] propertyArray = displayLine.Split();

                foreach (string propString in propertyArray)
                {
                    if (propString == "")
                        continue;

                    string[] propKeyAndValue = propString.Split(':');

                    if (propKeyAndValue.Count() == 0)
                        continue;

                    switch (propKeyAndValue[0])
                    {
                        case "byr":
                            newPassport.BirthYear = int.Parse(propKeyAndValue[1]);

                            if (newPassport.BirthYear < 1920 || newPassport.BirthYear > 2002)
                                newPassport.BirthYear = -1;

                            break;
                        case "iyr":
                            newPassport.IssueYear = int.Parse(propKeyAndValue[1]);

                            if (newPassport.IssueYear < 2010 || newPassport.IssueYear > 2020)
                                newPassport.IssueYear = -1;
                            break;
                        case "eyr":
                            newPassport.ExpirationYear = int.Parse(propKeyAndValue[1]);

                            if (newPassport.ExpirationYear < 2020 || newPassport.ExpirationYear > 2030)
                                newPassport.ExpirationYear = -1;
                            break;
                        case "hgt":
                            if (propKeyAndValue[1].Contains("cm"))
                            {
                            ...
                            break;
                        case "hcl":
                            newPassport.HairColor = propKeyAndValue[1];
                            Regex rx = new Regex(@"^#(?:[0-9a-f]{3}){2}$", RegexOptions.None);
                            bool isThereAMatch = rx.IsMatch(propKeyAndValue[1]);
                            if (isThereAMatch == false)
                            {
                                // Console.WriteLine(newPassport.HairColor);
                                newPassport.HairColor = "";
                            }
                            break;
                        case "ecl":
                            newPassport.EyeColor = propKeyAndValue[1];

                            Regex eyeRX = new Regex(@"^(amb|blu|brn|gry|grn|hzl|oth){1}$");

                            if (eyeRX.IsMatch(newPassport.EyeColor) == false)
                                newPassport.EyeColor = "";

                            break;
                        case "pid":
                            newPassport.PassportID = propKeyAndValue[1];

                            Regex pidRX = new Regex(@"^[0-9]{9}$");

                            if (pidRX.IsMatch(newPassport.PassportID) == false)
                                newPassport.PassportID = "";

                            break;
                        case "cid":
                            newPassport.CountryID = propKeyAndValue[1];
                            break;
                        default:
                            break;
                    }
            ...
            GoodPassports = AllPassports.Where(p => p.AllRequiredProps == true).ToList();
            BadPassports = AllPassports.Where(p => p.AllRequiredProps == false).ToList();

            int numberOfPassportsWithAllProps = AllPassports.Where(p => p.AllProps == true).ToList().Count();
            int numberOfPassportsWithReqiredProps = AllPassports.Where(p => p.AllRequiredProps == true).ToList().Count();
